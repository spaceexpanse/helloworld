...
private:
   ...
   * and is changed to CATRODNG_UP when a game_sendupdates request has been
   * sent to bring the game state up to the tip.
   *
   * CATRODNG_UP:  We are not at the daemon's current tip, and have requested
   ...
   * initial state to which we are catching up.  For CATRODNG_UP, this is
   * the TOBLOCK returned from game_sendupdates.
   *
   * This is compared against the RODLD hashes of block-attach notifications
   ...
   * CATRODNG_UP and requests game_sendupdates, or sets the state to
